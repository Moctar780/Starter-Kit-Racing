...
import { ConstraintType, MotionType, rigidBody, bvh, ShapeType } from 'crashcat';

var BodyColorMode;
(function (BodyColorMode) {
...
function ensureBatchedMeshGeometryCapacity(mesh, state, geometry) {
    ...
    const needsExpansion = state.bodies.currentVertexCount + requiredVertices > state.bodies.maxVertexCount ||
        state.bodies.currentIndexCount + requiredIndices > state.bodies.maxIndexCount;
    if (needsExpansion) {
        // grow conservatively
        const newMaxVertexCount = Math.max(state.bodies.maxVertexCount, state.bodies.currentVertexCount + requiredVertices + 5000);
...
function addShapeInstances(state, shape, bodyMatrix, localMatrix, color, instanceIds) {
    switch (shape.type) {
        ...
        case ShapeType.TRIANGLE_MESH: {
            ...
            const key = `triangle-mesh:${triangleMeshId}`;
        ...
        case ShapeType.CONVEX_HULL: {
            ...
            const key = `convex-hull:${convexHullId}`;
...
function updateBodiesBatchedMesh(state, world) {
    ...
    for (const body of rigidBody.iterate(world)) {
        ...
        _identityMatrix.identity();
        const index = { value: 0 };
        updateShapeInstances(state.bodies.batchedMesh, body.shape, _updateMatrix, _identityMatrix, instance.instanceIds, index, color);
    }
}
...
function ensureContactsSpheresCapacity(state, requiredCount) {
    if (requiredCount <= state.contacts.spheresMaxCount)
        return;
...
function ensureContactsCylindersCapacity(state, requiredCount) {
    if (requiredCount <= state.contacts.cylindersMaxCount)
        return;
...
function ensureContactsConesCapacity(state, requiredCount) {
    if (requiredCount <= state.contacts.conesMaxCount)
        return;
...
function ensureConstraintContactsSpheresCapacity(state, requiredCount) {
    if (requiredCount <= state.contactConstraints.spheresMaxCount)
        return;
...
function ensureConstraintContactsCylindersCapacity(state, requiredCount) {
    if (requiredCount <= state.contactConstraints.cylindersMaxCount)
        return;
...
function ensureConstraintContactsConesCapacity(state, requiredCount) {
    if (requiredCount <= state.contactConstraints.conesMaxCount)
        return;
...
function updateContacts(state, world) {
    ...
    if (state.contacts.spheresMesh.instanceColor)
        state.contacts.spheresMesh.instanceColor.needsUpdate = true;
    if (state.contacts.cylindersMesh.instanceColor)
        state.contacts.cylindersMesh.instanceColor.needsUpdate = true;
    if (state.contacts.conesMesh.instanceColor)
        state.contacts.conesMesh.instanceColor.needsUpdate = true;
}
function updateContactConstraints(state, world) {
    ...
    for (let i = 0; i < world.contactConstraints.count; i++) {
        const constraint = world.contactConstraints.pool[i];
        if (constraint.numContactPoints === 0)
            continue;
    ...
    for (let constraintIdx = 0; constraintIdx < world.contactConstraints.count; constraintIdx++) {
        ...
        for (let i = 0; i < constraint.numContactPoints; i++) {
            ...
            const hasImpulse = contactPoint.normalConstraint.totalLambda !== 0 ||
    ...
    if (state.contactConstraints.spheresMesh.instanceColor)
        state.contactConstraints.spheresMesh.instanceColor.needsUpdate = true;
    ...
    if (state.contactConstraints.conesMesh.instanceColor)
        state.contactConstraints.conesMesh.instanceColor.needsUpdate = true;
}
...
function updateTriangleMeshBvh(state, world) {
    ...
    if (showLeafNodes !== state.triangleMeshBvh.previousShowLeafNodes ||
...
function updateStaticCompoundBvh(state, world) {
    ...
    // invalidate cache if filter options changed
    if (showLeafNodes !== state.staticCompoundBvh.previousShowLeafNodes ||
...
function drawSwingConeLimits(positions, colors, center, twistAxis, planeAxis, normalAxis, planeHalfAngle, normalHalfAngle, radius, color) {
    ...
    for (let i = 0; i <= numSegments; i++) {
        ...
        const point = [center[0] + dirX * radius, center[1] + dirY * radius, center[2] + dirZ * radius];
...
function drawConstraints(state, world) {
    ...
    if (hingePool) {
        for (const constraint of hingePool.constraints) {
            ...
            addConstraintLine(positions, colors, pos1, [pos1[0] + axis1[0] * size, pos1[1] + axis1[1] * size, pos1[2] + axis1[2] * size], redColor);
            ...
            if (drawLimits && constraint.hasLimits && constraint.limitsMax > constraint.limitsMin) {
                vec3.set(_constraint_normal1, constraint.localSpaceNormalAxis1[0], constraint.localSpaceNormalAxis1[1], constraint.localSpaceNormalAxis1[2]);
    ...
    if (swingTwistPool) {
        for (const constraint of swingTwistPool.constraints) {
            if (constraint._pooled || !constraint.enabled)
                continue;
            const bodyA = getBodyTransform(constraint.bodyIndexA);
            const bodyB = getBodyTransform(constraint.bodyIndexB);
            ...
            if (drawLimits) {
                ...
                if (hasSwingLimits) {
                    drawSwingConeLimits(positions, colors, pos1, _constraint_twistAxis, _constraint_planeAxis, _constraint_normalAxis, constraint.planeHalfConeAngle, constraint.normalHalfConeAngle, size, greenColor);
                }
                // Draw twist limits as a pie around the twist axis
                const hasTwistLimits = constraint.twistMaxAngle > constraint.twistMinAngle;
                if (hasTwistLimits) {
                    drawPie(positions, colors, pos1, size, _constraint_twistAxis, _constraint_planeAxis, constraint.twistMinAngle, constraint.twistMaxAngle, purpleColor);
                }
            }
        }
    }
    ...
    if (distancePool) {
        for (const constraint of distancePool.constraints) {
            if (constraint._pooled || !constraint.enabled)
                continue;
            const bodyA = getBodyTransform(constraint.bodyIndexA);
            const bodyB = getBodyTransform(constraint.bodyIndexB);
            ...
            if (drawLimits && constraint.minDistance !== constraint.maxDistance) {
                ...
                const direction = [pos2[0] - pos1[0], pos2[1] - pos1[1], pos2[2] - pos1[2]];
    ...
    if (conePool) {
        for (const constraint of conePool.constraints) {
            if (constraint._pooled || !constraint.enabled)
                continue;
            const bodyA = getBodyTransform(constraint.bodyIndexA);
            const bodyB = getBodyTransform(constraint.bodyIndexB);
            ...
            addConstraintMarker(positions, colors, pos2, size * 0.1, greenColor);
            addConstraintLine(positions, colors, pos2, [pos2[0] + axis2[0] * size, pos2[1] + axis2[1] * size, pos2[2] + axis2[2] * size], greenColor);
            // Draw limits if enabled
    ...
    if (fixedPool) {
        for (const constraint of fixedPool.constraints) {
            if (constraint._pooled || !constraint.enabled)
                continue;
            const bodyA = getBodyTransform(constraint.bodyIndexA);
            const bodyB = getBodyTransform(constraint.bodyIndexB);
    ...
    if (pointPool) {
        for (const constraint of pointPool.constraints) {
            if (constraint._pooled || !constraint.enabled)
                continue;
            const bodyA = getBodyTransform(constraint.bodyIndexA);
            const bodyB = getBodyTransform(constraint.bodyIndexB);
    ...
    if (sliderPool) {
        for (const constraint of sliderPool.constraints) {
            if (constraint._pooled || !constraint.enabled)
                continue;
            const bodyA = getBodyTransform(constraint.bodyIndexA);
            const bodyB = getBodyTransform(constraint.bodyIndexB);
    ...
    if (sixDOFPool) {
        for (const constraint of sixDOFPool.constraints) {
            if (constraint._pooled || !constraint.enabled)
                continue;
            const bodyA = getBodyTransform(constraint.bodyIndexA);
            const bodyB = getBodyTransform(constraint.bodyIndexB);
            ...
            if (drawLimits) {
                ...
                // Get constraint space axes in world space
                const constraintTransform = { ...bodyA, quaternion: constraintQuat };
                const axisX = [0, 0, 0];
                const axisY = [0, 0, 0];
                const axisZ = [0, 0, 0];
                ...
                const axisColors = [redColor, greenColor, blueColor];
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    const isFree = minLimit <= -34e37 && maxLimit >= 3.4e38;
                    const isFixed = minLimit >= 3.4e38 && maxLimit <= -34e37;
                    if (!isFree && !isFixed && minLimit < maxLimit) {
                        ...
                        if (minLimit > -34e37 && maxLimit < 3.4e38) {
                            const minPos = [
                                pos1[0] + axis[0] * minLimit,
                                pos1[1] + axis[1] * minLimit,
                                pos1[2] + axis[2] * minLimit,
...
var debugRenderer = /*#__PURE__*/Object.freeze({
    __proto__: null,
    get BodyColorMode () { return BodyColorMode; },
    clear: clear,
...
export { createShapeHelper, debugRenderer };
